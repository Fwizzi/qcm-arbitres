-- =========================================================
-- Accélération du calcul de note : une seule requête groupée
-- =========================================================
-- calculer_score_question() fait 3 requêtes séparées PAR QUESTION.
-- submit_exam_attempt() l'appelle une fois par question du QCM : pour
-- un QCM à 20 questions, cela représente 60 petites requêtes à chaque
-- validation d'un arbitre, chacune avec son propre coût fixe — d'où la
-- lenteur ressentie sur les QCM à plusieurs questions.
--
-- Cette migration recalcule la note du QCM en UNE SEULE requête groupée
-- (au lieu d'itérer question par question), avec exactement la même
-- formule de notation. calculer_score_question() reste disponible pour
-- l'affichage question par question (correction détaillée), qui n'est
-- pas le goulot d'étranglement signalé ici.
...
begin
  ...
  -- Une seule requête groupée : le nombre de bonnes réponses attendues,
  -- le nombre coché et le nombre de bonnes réponses cochées, pour
  -- TOUTES les questions du QCM en une passe, plutôt qu'un aller-retour
  -- par question.
  with attendu as (
    select question_id, count(*) as nb_attendu
...
-- Même optimisation pour le recalcul en direct affiché sur les écrans
-- de résultat (appelé à chaque affichage, potentiellement souvent).
